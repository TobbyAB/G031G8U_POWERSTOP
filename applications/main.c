...
void powerstop_flag_handle(){
    if(powerstop_flag == 0){
        ...
        if(rt_pin_read(WATER_SINGAL) == PIN_LOW && powerstop_err != 2){
            powerstop_test = 0;
            powerstop_err = 2;
        ...
        if(rt_pin_read(SW_CTRL) == PIN_LOW){
            if(rt_pin_read(WATER_SINGAL) == PIN_HIGH){
                if(powerstop_test == 0 ){
                    if(rt_pin_read(HALL_SINGAL1) == PIN_HIGH || rt_pin_read(HALL_SINGAL2) == PIN_HIGH || powerstop_err == 2)
                    powerstop_test = 1;
                }
            }
        }
...
void powerstop_error_handle(){
    if(powerstop_err_value != powerstop_err){
        powerstop_err_value = powerstop_err;
        powerstop_err_time = 0;
    }
    if(powerstop_err == 0){
        if(powerstop_test == 0)set_led_GreenOn();
        buzz_off();
    }else if(powerstop_err == 1){

        powerstop_err_time %= 48;
    ...
    }else if(powerstop_err == 2){

        powerstop_err_time %= 52;
    ...
    }else if(powerstop_err == 3){

        powerstop_err_time %= 64;
